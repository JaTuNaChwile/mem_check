...
{
  memory_[mem_idx].filename = file;
  memory_[mem_idx].line = line;
  memory_[mem_idx].func_name = func;
  mem_idx++;
}
...
void mem_scanner(void)
{
  for(size_t i = 0; i<DEFAULT_MEMORY_CALLS; i++)
  {
    ...
    {
      fprintf(stderr, "Memory at address 0x%llx was not free'd. At file %s:%d in"
      " function %s\n", memory_[i].ptr, memory_[i].filename,memory_[i].line,
         memory_[i].func_name);
    }
  }
